...
function shadowGapFor(_index: number) {
  return 0.1;
}
...
function useNormalized(url: string, framing: Framing) {
  ...
  return useMemo(() => {
    ...
    return { object: wrapper, bottomY, parts, meshToPart };
...
function Model({
  ...
  const { object, bottomY, parts, meshToPart } = useNormalized(
    url,
    framingFor(url),
  );
